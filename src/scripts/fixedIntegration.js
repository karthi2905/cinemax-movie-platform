...
const MAX_MOVIES_TO_ADD = 500; // Even more conservative number
...
// Create clean movies file with only essential movies
function createCleanMoviesFile() {
  return `const movies = [
  {
    id: "1",
    ...
    cast: [
      ...
      { name: "Morgan Freeman", character: "Ellis Boyd Redding", image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop" }
    ],
...
async function fixedIntegration() {
  try {
    console.log('🎬 Starting Fixed CineMax Dataset Integration...');
    console.log(`📊 Target: Add ${MAX_MOVIES_TO_ADD} carefully selected dataset movies`);
    
    // Create clean movies content
    console.log('🔄 Creating clean movies structure...');
    let cleanMoviesContent = createCleanMoviesFile();
    ...
    // Add the dataset movies to the clean structure with proper escaping
    const newMoviesCode = convertedMovies.map(movie => {
      const safeTitle = escapeString(movie.title);
      const safeSynopsis = escapeString(movie.synopsis);
    ...
    console.log('💾 Writing fixed movies file...');
    fs.writeFileSync(MOVIES_FILE, updatedContent, 'utf8');
    
    console.log('🎉 Fixed integration completed successfully!');
    ...
    console.error('❌ Error in fixed integration:', error);
    return {
      success: false,
      error: error.message
    };
